...
/*Add code management uri*/
static char *url_uri = NULL;
static char *url_unparsed_uri = NULL;
...
static int access_checker(request_rec *r) 
{
    ...
    /*Uri management*/
    if ( ( url_uri != NULL) || ( url_unparsed_uri != NULL) ) {
	if (url_unparsed_uri != NULL){
		result = strcmp(url_unparsed_uri,r->unparsed_uri);
		if ( result != 0){
			if (url_uri != NULL){
				result = strcmp(url_uri,r->uri);
			}
		}
	}else{
		if (url_uri != NULL){
    ...
    /* If the uri give by sysadmin in httpd.conf is equals with uri take by r->unparded_uri, we go on mod_evasive*/ 
    //if ( ( result == 0 ) || ( (url_uri == NULL) && (url_unparsed_uri == NULL) ) ) {
    if ( result == 0 ) {
...
/* Add code uri managements , url_uri
 * and *url_unparsed_uri
 **/
static const char *
get_url_uri(cmd_parms *cmd, void *dconfig, const char *value) {
  if (value != NULL && value[0] != 0) {
    if (url_uri != NULL)
      free(url_uri);
    url_uri = strdup(value);
  }

  return NULL;
}
static const char *
get_url_unparsed_uri(cmd_parms *cmd, void *dconfig, const char *value) {
  if (value != NULL && value[0] != 0) {
    if (url_unparsed_uri != NULL)
      free(url_unparsed_uri);
    url_unparsed_uri = strdup(value);
  }
...
static const command_rec access_cmds[] =
{
	AP_INIT_TAKE1("DOSHashTableSize", get_hash_tbl_size, NULL, RSRC_CONF, 
	    ...
	    /* Add url management  */
        AP_INIT_TAKE1("DOSGeturi", get_url_uri, NULL, RSRC_CONF,
                "Set char uri"),

        AP_INIT_TAKE1("DOSGeturi_unparsed", get_url_unparsed_uri, NULL, RSRC_CONF,
                "Set unparsed uri"),
